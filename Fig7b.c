...
int main()
{
    ...
    //double rhog=0.1/vol;
    double rho_ideal=1.0e-06; //4.1e-06;
    ofstream outfile;
    //ofstream outfile1;
    outfile.open ("Fig7b.txt");
    ...
    for (int counter_r=0; counter_r<rmax; counter_r++)
    {
        ...
        for (int counter_DG0=0; counter_DG0 < DG0max; counter_DG0++)
        {
            ...
            for (int counter_N=0; counter_N<Nmax; counter_N=counter_N+1)
            {
                ...
                DG0_star_surface=DG0_star-1; //+counter_N;
                //N=2+counter_N;
                N=80;
                NA=N/2;
                NB=N/2;
...
//                    exit(0);
                    nfS_C=rand()% NsC+1;
                    flag=true;
                    //This part is for fixed point interation when film thickness is hmax
                    while (flag==true)
                    {
                     ...
                     //End of the part: fixed point interation when film thickness is hmax
                    ...
                    else 
                    {
                        ...
                        F[hmax]=exp(-beta*Free_energy_diff)*pow((e*rho_ideal*X*v0),hmax);
                    }
...
//                    outfile << hmax << " " << exp(-betaFreeEnergy+hmax*betaFreeEnergy_bulk) << endl;
                ...
                //outfile << DG0_star << " " << DG0_star_surface << "  " << gamma << " " << DG0_star-DG0_star_surface << endl;
