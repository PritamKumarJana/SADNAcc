...
int main()
{
    ...
    //double rhog=0.1/vol;
    double rho_ideal=4.1e-06; //1.38e-07;//7.0e-04; //  4.1e-06;
    //ofstream outfile;
    //ofstream outfile1;
    //outfile.open ("Phase_diagram_surface_fcc.dump");
    ...
    for (int counter_r=0; counter_r<rmax; counter_r++)
    {
        ...
        for (int counter_DG0=0; counter_DG0 < DG0max; counter_DG0++)
        {
            ...
            for (int counter_N=0; counter_N<Nmax; counter_N=counter_N+1)
            {
                ...
                NA=N/2;
                NB=N/2;
                NsC=189; //215; //+counter_N*10.0;
                //rho_ideal=0.000000000001 + 0.000000000001*counter_N;
                ...
                for (hmax=hmax_; hmax<=hmax_; hmax++)   //hmax_
                {
                    for (h=0; h<hmax; h++)
                    {
...
//                    exit(0);
                    ...
                    for (h=0; h<hmax; h++) 
                    {
                        ...
                        //cout << "h " << h << endl;
                        cout << "layer number= " << h+1 << " Lateral= " << (P[h].nf_A * 6 * P[h].nf_B + P[h].nf_B * 6 * P[h].nf_A) * qb_colloid << endl;
                        
                        if (h==0) cout << "layer number= " << h+1 << " vertical= " << (P[h].nf_A * 3 * P[h+1].nf_B + P[h].nf_B * 3 * P[h+1].nf_A) * qb_colloid + P[h].nf_A * nfS_C * qb_surface << endl;
                        
                    
//                        //if (h==0) cout << "layer number= " << h << " " <<  P[h].nf_A * nfS_C * qb_surface << endl;
                        else if (h==2) cout << "layer number= " << h+1 << " vertical= " << P[h].nf_A * 3 * ( P[h-1].nf_B) * qb_colloid +  P[h].nf_B * 3 * (P[h-1].nf_A) * qb_colloid << endl;
                        
                        else cout << "layer number= " << h+1 << " vertical= " << P[h].nf_A * 3 * (P[h+1].nf_B + P[h-1].nf_B) * qb_colloid +  P[h].nf_B * 3 * (P[h+1].nf_A + P[h-1].nf_A) * qb_colloid << endl;
                        
                        //if (h==0) cout << "layer number= " << h << " Free = " << NA*log((P[h].nf_A*P[h].nf_B)/(NA*NB)) + (2*NA-P[h].nf_A-P[h].nf_B)/2.0 + (NsC-nfS_C)/2.0 + NsC*log(nfS_C/(double)NsC) - FreeEnergy_bulk <<  " " << NA*log((P[h].nf_A*P[h].nf_B)/(NA*NB)) << " " << " " << P[h].nf_A << " " << P[h].nf_B << " " << (2*NA-P[h].nf_A-P[h].nf_B)/2.0+(NsC-nfS_C)/2.0 << " " << (P[h].nf_A*P[h].nf_B)*ql << (NsC-nfS_C)/2.0 << " " << NsC << " " << log(nfS_C/(double)NsC) << endl;
                        
                        //NA*log((P[h].nf_A*P[h].nf_B)/(NA*NB)) + (2*NA-P[h].nf_A-P[h].nf_B)/2.0 + (NsC-nfS_C) + NsC*log(nfS_C/(double)NsC) 
                        //else if (h==1) cout << "layer number= " << h << " Free = " <<  NA*log((P[h].nf_A*P[h].nf_B)/(NA*NB)) + (2*NA-P[h].nf_A-P[h].nf_B)/2.0 -FreeEnergy_bulk << endl;
                        
                        //else if (h==2) cout << "layer number= " << h << " Free = " <<  NA*log((P[h].nf_A*P[h].nf_B)/(NA*NB)) + (2*NA-P[h].nf_A-P[h].nf_B)/2.0 -FreeEnergy_bulk << endl;
