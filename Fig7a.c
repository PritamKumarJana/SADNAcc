...
int main()
{
    int N;
    int NA,NB,NsC;
    double X=1.0; //0.5 and 2.0 to obtain the lower and upper error, respectively
    double beta=1.0;
    int hmax_=10;
    double F[maxh]={0.0};
    int hmax;
    ...
    //double rhog=0.1/vol;
    double rho_ideal=0.000001;
    ofstream outfile;
    //ofstream outfile1;
    outfile.open ("Fig7a.txt");
    ...
    for (int counter_r=0; counter_r<rmax; counter_r++)
    {
        ...
        for (int counter_DG0=0; counter_DG0 < DG0max; counter_DG0++)
        {
            ...
            for (int counter_N=0; counter_N<Nmax; counter_N=counter_N+1)
            {
                ...
                rho_ideal= 1e-12 + 9.999999e-10*counter_N; //   1e-12 + 9.999999e-10*counter_N;
...
//                    outfile << hmax << " " << exp(-betaFreeEnergy+hmax*betaFreeEnergy_bulk) << endl;
                ...
                outfile << rho_ideal*e << " " << gamma << " " << N << " " << DG0_star_surface << endl;
